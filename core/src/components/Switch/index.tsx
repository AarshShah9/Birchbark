...
const Switch = ({ className, value, setValue }: SwitchProps) => (
  <div className={`inline-flex shrink-0 ${className}`}>
    <SwitchReact
      checked={value}
      onChange={setValue}
      className={`relative inline-flex h-6 w-12 cursor-pointer rounded-full border-2 border-transparent outline-none transition-colors ${
    ...
    >
      <span
        aria-hidden="true"
        className={twMerge(
          `pointer-events-none inline-block h-5 w-5 rounded-full bg-n-1 transition-all dark:bg-n-7 ${
